param(
  [string]$SourceCsv = "dpwas.csv",
  [string]$OutputFile = "api/dpwas.secure.json",
...
foreach ($row in $rows) {
  ...
  $dateProperty = $rowObject | Where-Object {
    ($_.Name.ToLowerInvariant().Trim()) -in @("date")
  } | Select-Object -First 1

  $timeProperty = $rowObject | Where-Object {
    ($_.Name.ToLowerInvariant().Trim()) -in @("time")
  } | Select-Object -First 1

  if ($null -eq $appProperty -or $null -eq $dateProperty -or $null -eq $timeProperty) {
    throw "CSV must include Application No., Date, and Time columns."
  }

  $normalizedAppNo = Normalize-AppNo -Value ([string]$appProperty.Value)
  $date = ([string]$dateProperty.Value).Trim()
  $time = ([string]$timeProperty.Value).Trim()

  if ([string]::IsNullOrWhiteSpace($normalizedAppNo) -or [string]::IsNullOrWhiteSpace($date) -or [string]::IsNullOrWhiteSpace($time)) {
    continue
  }
